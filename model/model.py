...
class Projector(nn.Module):
    def __init__(self, bn_dim=256):
        super(Projector, self).__init__()
        self.projector_0 = nn.Linear(2048, bn_dim)
        self.projector_0.weight.data.normal_(0, 0.005)
        self.projector_0.bias.data.fill_(0.1)
        self.projector_layer = nn.Sequential(self.projector_0, nn.BatchNorm1d(bn_dim), nn.ReLU())

    def forward(self, x):
        output = self.projector_layer(x)
        return output


class Classifier(nn.Module):
    def __init__(self, input_num, cate_num):
        super().__init__()
        self.classifier = nn.Linear(input_num, cate_num)
...
#         self.bottleneck_0 = nn.Linear(2048, bn_dim)
#         self.bottleneck_0.weight.data.normal_(0, 0.005)
#         self.bottleneck_0.bias.data.fill_(0.1)
#         self.bottleneck_layer = nn.Sequential(self.bottleneck_0, nn.BatchNorm1d(bn_dim), nn.ReLU())
#         self.total_classes = total_classes
...
#
#     def forward(self, x):
#         out_bottleneck = self.bottleneck_layer(x)
#         if not self.total_classes:
#             return (out_bottleneck, None)
#         logits = self.classifier_layer(out_bottleneck)
#         return (out_bottleneck, logits)
